    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if ($this-><%= $currentModelName; %>->save($<%= $singularName %>)) {
                $this->Flash->success(__('The <%= strtolower($singularHumanName) %> has been saved.'), ['plugin' => 'Template']);

                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error(__('The <%= strtolower($singularHumanName) %> could not be saved. Please, try again.'), ['plugin' => 'Template']);
        }
<%
